...
import { ArrowRight, Building, Pencil, Plus, Search, Trash2, X } from 'lucide-react';
...
/**
 * /marcas — Rediseño UI (look de MarcasView).
 *
 * SOLO UI/UX. La lógica de datos es idéntica a la versión previa
 * (page-f3aad5e0): `listBrandsPaginated({ q, page, pageSize })` con filtros
 * en URL, CRUD vía createBrand/updateBrand/deleteBrand, ConfirmDialog para
 * borrado y toasts con apiErrorMessage.
 *
 * Cambios visuales (sistema compartido con Categorías / Almacenes):
 *  · Header font-black + subtítulo con stats.
 *  · "Sheet" rounded-3xl con tabla; fila con Building azul, nombre en
 *    uppercase black, Chevron 'Abrir' y acciones en hover.
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <tr>
                  <td colSpan={2} className="py-14 text-center">
                    <p className="text-sm font-bold text-slate-600 dark:text-slate-300">
                      {debouncedQ ? 'Sin resultados para tu búsqueda' : 'No hay marcas cargadas'}
                    </p>
              ...
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-slate-400">
                  {editing
                    ...
                    : 'Registrá una nueva marca en el sistema'}
                </p>
              </div>
              <button
                onClick={closeDialog}
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget ? (
            <>
              Se eliminará <strong>{deleteTarget.name}</strong>. Si tiene productos
              asociados la operación va a fallar — primero reasigná o eliminá los
              productos.
            </>
          ) : null
        }
...
/* ============================================================
   BrandRow — fila con Building, nombre uppercase black y acciones hover
   ============================================================ */
function BrandRow({
  brand,
  onEdit,
  ...
  return (
    <tr className="group transition-colors hover:bg-slate-50/60 dark:hover:bg-slate-800/10">
      <td className="py-4 pl-6 pr-3">
        ...
        >
          <Building className="h-4 w-4 shrink-0 text-[#2F6BFF] transition-transform group-hover:scale-105 dark:text-blue-400" />
