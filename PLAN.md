...
1. **Migración** que desglosa `customers.address` en `addressStreet`, `addressNumber`, `addressCommune` (formato chileno). Decisión a confirmar con cliente: separar columnas vs. dejar texto libre — ver [Decisiones pendientes con el cliente](#decisiones-pendientes-con-el-cliente).
2. **Migración** que agrega índice único en `customers.taxId` (RUT) y `suppliers.taxId` (NIT/RUC).
3. CRUD de `Customer`. Campos: **RUT/ID**, nombre, correo, dirección (calle, número, comuna), teléfono, notas internas.
4. CRUD de `Supplier` ya existe (Fase 3) — falta refinar la pantalla con detalle/tabs y subir validación de RUT al nivel de DB.
5. Detalle de cliente con tabs *Datos* / *Cotizaciones* / *Ventas*.
6. Detalle de proveedor con historial de entradas.
7. Validación de teléfono en formato internacional (clave para WhatsApp).
8. Validación de RUT chileno (formato + dígito verificador) — confirmar con cliente si querés validación estricta o solo formato.
...
| 2 | "Mismo producto con mismo código" (frase del cliente) — ¿permitir duplicados de SKU? ¿códigos compartidos entre productos? ¿fusionar productos con mismo código? | 4B | `sku` se mantiene único; `product_codes` permite que distintos productos compartan el mismo `code` (ej. universal compartido por equivalentes). |
| 3 | Comisión por tarjeta: ¿% fijo, por método (débito vs crédito), por venta individual? | 5 / 7 | `cardCommissionRate` único en `CompanySettings` aplicado a todas las ventas con `paymentMethod=CARD`. |
| 4 | Tasa de IVA: ¿19% Chile fijo o configurable? | 5 | Configurable, default `0.19`. |
| 5 | Mercado Libre: ¿integración real con API ML (OAuth + sync de stock y órdenes) o solo registro manual de transferencias y ventas por bodega? | 7.5 | Manual. Integración real sería un proyecto aparte. |
| 6 | Almacenamiento de archivos (foto producto, factura compra): ¿disco local del servidor, S3, Cloudinary? | 4B / 5 | Disco local en `apps/api/uploads/` durante MVP. |
| 7 | Guía de despacho: ¿requiere numeración propia y entidad? ¿requisitos legales SII (Chile)? | 7.7 | Entidad `DispatchNote` con número correlativo. Sin emisión SII todavía. |
| 8 | Impresión 80mm: ¿es para impresora térmica POS conectada al PC o solo "imprimir desde el navegador" en formato angosto? | 6 / 7 / 7.7 | HTML con `@page` 80mm — el navegador maneja la impresora. |
| 9 | Validación de RUT chileno: ¿formato + dígito verificador estricto, o solo formato? | 4 | Formato + dígito verificador. |
| 10 | Cotización por WhatsApp: ¿el "número" del que habla el cliente ("cargar productos luego número y enviar") es el del cliente o el correlativo de la cot? | 6 | Asumimos que el número correlativo se asigna al guardar y el botón WhatsApp pre-arma el mensaje al teléfono del cliente.|
